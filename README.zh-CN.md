...
打开 **Phone Information V2**（`*#*#4636#*#*` 里的新无线电测试页；V1 是旧页）也会把覆盖清掉。那是系统测试界面，有权调用 `overrideConfig(..., null)` 回到运营商 XML。Ims 拦不住。

补丁因为上述原因丢了之后，不必再重启。Ims 会在运营商配置变化时，以及大约每 15 分钟的低功耗 Job 里重读 5G+ 键；键没了就会像刚开机一样再出通知。打开无线调试，在通知栏确认 Apply 即可。
